...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    static final int spread(int h) {
        return (h ^ (h >>> 16)) & HASH_BITS; //高位 0  目的是算出来是正数
    }
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            //正在扩容的移动阶段
                ...
                tab = helpTransfer(tab, f);//cas操作
            else {//i位置已经有值了 用链表或者红黑树
                V oldVal = null;
                synchronized (f) {//同步锁 当前节点
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) { //是否为链表
                            binCount = 1;
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            //sizeCtl 是tab扩容和初始化的控制器，默认是0，可以进行操作，
            // 负的话就代表正在初始化或扩容，
            // 因为可以多个线程扩容，-N 就代表n个线程正在扩容
            if ((sc = sizeCtl) < 0)
                Thread.yield(); // lost initialization race; just spin
                //CAS 设置 sizectl 设为-1，失败会跳过 让出cpu资源
            //判断是否需要扩容，然后尝试是否可以扩容
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        for (;;) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                h = (int) ThreadLocalRandom.advanceProbe(h);
    ...
    private final void treeifyBin(Node<K,V>[] tab, int index) {
        Node<K,V> b; int n, sc;
        if (tab != null) {
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY)//数组小于64不变树
                tryPresize(n << 1);
